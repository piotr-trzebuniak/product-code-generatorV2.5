import { featuresMapEN } from "../../../featureMaps/featuresMapEN";
  ...
  return ingredientsTable
    .map((ingredient) => {
      ...
      let ingredientName = `<strong>${
        ingredient.ingredient?.en || ""
      }</strong>`;
      let ingredientValue = ingredient.ingredientValue?.en || ""; // Sprawdzamy, czy ingredientValue ma właściwość 'pl'
      ...
      if (
        ...
        ingredient.ingredientValue.en
      ) {
        ingredientValue = ingredient.ingredientValue.en; // Wyciągamyy wartość 'pl'
      }
      ...
      if (ingredient.additionalLines && ingredient.additionalLines.length > 0) {
        ingredient.additionalLines.forEach((line) => {
          ingredientName += `<br>${line.ingredient?.en || ""}`;

          // Obsługuje przypadek, gdy line.ingredientValue jest obiektem
          let additionalIngredientValue = line.ingredientValue?.en || "";
          if (
            typeof line.ingredientValue === "object" &&
            line.ingredientValue.en
          ) {
            additionalIngredientValue = line.ingredientValue.en;
          }
...
export function removeTrailingBracketAndDots(htmlString) {
  let result = htmlString;

  // Usuń dodatkowy > jeśli występuje podwójnie na końcu
  if (result.endsWith('>>')) {
    result = result.slice(0, -1);
  }

  // Usuń niepotrzebne kropki na końcu (jeśli występują po znaczniku HTML lub samodzielnie)
  // Przykład: "<p>Tekst</p>." -> "<p>Tekst</p>"
  result = result.replace(/([>])\.+$/, '$1'); // usuwa kropki po znaczniku >
  result = result.replace(/\.+$/, '');        // usuwa same kropki, jeśli są na końcu
...
function extractIngredientsAndRemove(htmlString) {
  ...
  const pattern = /<h3><strong>(Ingredients|Engredients):<\/strong><\/h3><p>([^<]*)<\/p>/i;
  
  // Szukaj dopasowania
  const match = htmlString.match(pattern);
  
  if (!match) {
    console.log('Nie znaleziono nagłówka "Ingredients:" lub składników');
    return {
      extractedText: '',
...
export const generateFeatureHtml = (specialFeatures, featuresMapEN) => {
  ...
  const enabledFeatures = Object.entries(specialFeatures).filter(
    ([key, value]) => value && featuresMapEN[key]
  );
  ...
  return enabledFeatures
    .map(
      ([key]) => `
        <div class="property">
          <img
            src="${ICON_BASE_URL}${featuresMapEN[key].icon}"
            alt=""
            srcset=""
          />
          <span>${featuresMapEN[key].label}</span>
        </div>`
    )
    .join("");
};
...
export const generateEbayEnHtmlCosmetics = (productData) => {
  if (
    !productData ||
  ...
  const editedCosmeticsDescription4 = extractIngredientsAndRemove(productData.cosmeticsDescription4.en)
    ...
    <div class="container container__header">
      ...
      <nav class="header__nav">
        <a
          ...
          href="https://www.ebay.co.uk/str/medpaksupplements?_tab=feedback"
          >Seller Feedback</a
        >
        <a
          for="tab2-btn"
          class="header__nav-item"
          href="https://www.ebay.co.uk/str/medpaksupplements"
        >
        ...
        <a
          for="tab4-btn"
          class="header__nav-item"
          href="https://www.ebay.co.uk/str/medpaksupplements?_tab=about"
          >Contact seller</a
        >
      </nav>
    </div>
        ...
        <div class="top-section">
          <div class="container container__top-section">
            <div class="product-name">
              <h2>${productData.productName.en}</h2>
            </div>
            <div class="gallery">
            ...
            <div class="right-side">
              ...
              <div class="roles">
                ${generateRoleHtml(productData.cosmeticsDescription3.en)}
              </div>
              </div>
                ...
                <div class="row">
                  <div class="col-md-6">
                    ...
                    ${removeTrailingBracketAndDots(productData.cosmeticsDescription2.en)}
                     ${productData.ingredientsTable[0].ingredient.en !== "" ? `
            <div class="table-responsive">
              <table class="table table-hover">
              <thead class="table-lighter">
                <tr>
                  <th>Ingredients</th>
                  <th>${productData.portion.portionAmount} ${productData.portion.unit.en}</th>
                  <th>RI</th>
                </tr>
              </thead>
              <tbody>
...
<!-- 
            ...
            <div class="product-series section">
              ...
              <div class="product-series__content">
                <a href="https://www.ebay.co.uk/str/medpaksupplements/Dietary-Supplements/_i.html?store_cat=36534928016" class="product-series__box">
                  <img
